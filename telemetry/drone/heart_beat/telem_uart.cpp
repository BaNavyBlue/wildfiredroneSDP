...
int main (int argc, char **argv){
    ...
    buffer[7] = package.msgid = IMG_SEND;
    buffer[8] = 0;
    buffer[9] = 0;
...
void send_image(char **argv, thread_data *shared, uint16_t comp_id, mav_data *package, uint8_t *buffer){
    ...
    if(p_fd){
        ...
        buffer[1] = strlen(argv[1]) + 3; // filename plus newline plus 2 bytes
        buffer[10] = packets;
        buffer[11] = packets >> 8;
        strcpy((char*)&buffer[12], argv[1]);
    ...
    while ((packet_cnt < packets) || (shared->flags & WAIT_FOR_FINISHED_ACK)){
        ...
        if(!(shared->flags & RECEIVED_START_ACK)){
            ...
            package->payload[0] = package->payload[13];
            package->payload[1] = package->payload[14];
            printf("index: %u\n", index);
            ssize_t bytes_read = pread(p_fd, &package->payload[2], 253, index * 253);
            ...
            write_packet(buffer, shared); 
        } else if(!(shared->flags & SENT_FINAL_PACKET)){
