#ifndef TELEM_DEFINES_H
#define TELEM_DEFINES_H

#include <iostream>
#include <cstdlib>
#include <cmath>
#include <inttypes.h>
#include <fstream>
#include <signal.h>
#include <time.h>
#include <sys/time.h>
#include <mavlink.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <fcntl.h>
#include <unistd.h>
#include <termios.h>
#include <errno.h>
#include <stdint.h>
#include <vector>
#include <checksum.h>
#include <pthread.h>
#include <assert.h>

#define BUFFER_SIZE 64
#define ZERO 0           // Guess
#define BYTES_P_TRANS 1  // Currently byte by byte
#define DELAY 160         // Tx Delay
#define MAX_PACKET 280 // Max Mavlink Packet
#define BASE_SIZE 12 // Min package size with no payload
#define MAGIC 0xFD  // MAVLink v2
#define MAVGIC 0xFE // MAVLink v1
#define HEADER_L 10
#define START_PACKET 0 
#define FILE_PACKET 1
#define LAST_PACKET 2
#define RESENT_PACKET 3
#define PACKAGE_SIZE 256
#define SYS_ID_DRONE 1
#define COMP_ID_CAMERA 100
#define COMP_ID_LWIR   101

enum MSG_ID{
    HEARTBEAT,
    ACK_MSG = 77,
    SEND_IMG = 99,
    RAD_MSG = 109
};

enum EXT_CRC{
    SEND_IMG_CRC = 0,
    H_BEAT_CRC = 50,
    ACK_CRC = 143
};

typedef struct {
    uint8_t magic; // Packet Start
    uint8_t len; // Payload length
    uint8_t incompat_flags; // Incompatibility Flags
    uint8_t compat_flags;  // Compatibility Flags
    uint8_t seq; // Packet sequence number 0-255 duh
    uint8_t sysid; // System ID (Sender) 1-255
    uint8_t compid; // component ID (Sender) 1_255
    uint32_t msgid:24; // Message ID (low, middle, high, bytes)
    uint8_t payload[255]; // PTR to message data 255 bytes at a time max
    uint16_t checksum;
    uint8_t signiture[13];
}mav_data;

typedef struct {
    double t_ms;
    char clock[100];
} time_data;

// Thread Flags (BIT MASKING)
#define RECEIVED_START_ACK 0x0001 // Ready to start sending file payload
#define SENT_START_ACK 0x0002 // Waiting for payload packets to start
#define WAIT_FOR_FINISHED_ACK 0x0004 // Will not end sending function until ACK
#define REPLACE_PACKET_FLAG 0x0008 // Send Replacement packet
#define SENT_FINAL_PACKET 0x0010 // We sent final packet in file.
#define RECEIVING_FILE 0x0020 // Waiting for file to complete.
#define HEART_BEAT_PRESENT 0x8000 // Is other device still available

typedef struct {
    pthread_mutex_t *uart_mut;
    pthread_mutex_t *index_mut;
    int *fd;
    int *recv_fd;
    uint16_t flags = 0;
    uint16_t replace_pckt = 0;
    time_data *my_time;
}thread_data;

#include "heart_beat.h"
#include "time_keeper.h"
#include "telem_uart.h"
#include "uart_recv.h"
#include "write_helper.h"
#include "host_config.h"
#include "send_ack.h"
#include "request_client.hpp"
#include "uart_server.hpp"
#endif
